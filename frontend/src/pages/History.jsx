...
export default function History({token, onOpenProject}) {
  ...
  return (
    <section className="relative mx-auto w-full max-w-5xl px-5 py-10">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-10 left-1/2 h-48 w-[36rem] max-w-full -translate-x-1/2 rounded-full bg-neutral-800/30 blur-3xl"
      />
      ...
      {loading ? <p className="text-sm text-neutral-500">Loading projects...</p> : projects.length === 0 ? (
        <p className="rounded-xl border border-dashed border-neutral-800 bg-neutral-900/40 p-5 text-sm text-neutral-500">Your generated projects will appear here.</p>
      ) : (
      ...
      {editingProject && (
        <form onSubmit={saveTitle} className="relative mt-6 flex max-w-lg gap-2 rounded-xl border border-neutral-800 bg-neutral-900 p-4">
